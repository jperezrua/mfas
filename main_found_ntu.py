...
def parse_args():
    ...
    parser.add_argument('--datadir', type=str, help='data directory', default='/home/juanma/Documents/Data/ROSE_Action/')
    ...
    parser.add_argument('--inner_representation_size', type=int, help='output size of mixing linear layers', default=256)
...
def get_dataloaders(args):
    ...
    dataloaders = {x: DataLoader(datasets[x], batch_size=args.batchsize, shuffle=True, num_workers=args.num_workers, drop_last=False, pin_memory=True) for x in ['train', 'dev', 'test']}
...
def train_model(rmode, configuration, dataloaders, args, device):
    ...
    if args.test_cp == '':
      ...
      interm_model_acc = tr.train_ntu_track_acc(rmode, criteria, optimizer, scheduler, dataloaders, dataset_sizes, 
                  device=device, num_epochs=1, verbose=args.verbose, multitask=args.multitask)
      ...
      best_model_acc = tr.train_ntu_track_acc(rmode, criteria, optimizer, scheduler, dataloaders, dataset_sizes, 
                  device=device, num_epochs=args.epochs, verbose=args.verbose, multitask=args.multitask)    

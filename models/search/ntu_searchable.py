    ...
    for idx,configuration in enumerate(sampled_configurations):
        ...
        if not return_model or idx in return_model:
            ...
            if not preaccuracies:
                best_model_acc = tr.train_ntu_track_acc(rmode, criterion, optimizer, scheduler, dataloaders, dataset_sizes, 
                        device=device, num_epochs=args.epochs, verbose=args.verbose)
            else:
                best_model_acc = tr.train_ntu_track_acc(rmode, criterion, optimizer, scheduler, dataloaders, dataset_sizes, 
                        device=device, num_epochs=args.epochs, verbose=args.verbose, init_f1=preaccuracies[idx])      

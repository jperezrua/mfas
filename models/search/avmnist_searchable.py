...
def train_sampled_models(sampled_configurations, searchable_type, dataloaders, 
    ...
    for idx,configuration in enumerate(sampled_configurations):
        ...
        if not return_model or idx in return_model:
            ...
            best_model_acc = tr.train_avmnist_track_acc(rmode, [criterion], optimizer, scheduler, dataloaders, dataset_sizes, 
...
class Searchable_Audio_Image_Net(nn.Module):
    ...
    def _create_alphas(self):
        ...
        sizes_aud = [int(self.args.channels),int(self.args.channels*2),int(self.args.channels*4),int(self.args.channels*8),int(self.args.channels*16)] #sizes last 4 (-1)

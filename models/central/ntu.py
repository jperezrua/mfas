...
class Skeleton(nn.Module): #https://arxiv.org/pdf/1804.06055.pdf  https://github.com/huguyuehuhu/HCN-pytorch/blob/master/model/HCN.py
    def __init__(self,args):
...
class CentralNet(nn.Module):
    def __init__(self, args):
        ...
        # Central
        central_list = [nn.Sequential(*[nn.Conv2d(conv_dim, 2*conv_dim, kernel_size=4, stride=2, padding=1),torch.nn.BatchNorm2d(2*conv_dim),nn.ReLU(args)]) for conv_dim in [512]]
        central_list.append(nn.Sequential(*[nn.Conv2d(1024, 2048, kernel_size=4, stride=2, padding=1),torch.nn.BatchNorm2d(2048),nn.ReLU(args), nn.AvgPool2d((7,7))])) 
    ...
    def forward(self, input):
        ...
        for c_conv, mv, ma, aa, av, ac in zip(self.central_conv, [out_2, out_3, out_5, visual_pred], [hidden[1], hidden[2], hidden[-1], skel_pred], self.alphas_a, self.alphas_v, self.alphas_c):
